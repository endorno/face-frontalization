...
class ThreeD_Model:
    ...
    def default_model():
        path = get_file(
            'model3Ddlib.mat',
            'https://github.com/dougsouza/face-frontalization/raw/master/frontalization_models/model3Ddlib.mat',
            'abe9b2356834bb4ab56c6825283c5420',
            cache_subdir='face_models'
        )
        return ThreeD_Model(path, 'model_dlib')
...
class EyeMask:
    ...
    def get_default():
        f_path = get_file(
            'eyemask.mat',
            'https://github.com/dougsouza/face-frontalization/raw/master/frontalization_models/eyemask.mat',
            '53c4aa68efe36e75a53cd41efcc70f0f',
            cache_subdir='face_models'
        )
        return np.asarray(scio.loadmat(f_path)['eyemask'])
...
def frontalize(img, proj_matrix, ref_U, eyemask):
    ...
    ind = np.ravel_multi_index(
        (np.asarray(temp_proj2[1, :].round(), dtype='int64'), np.asarray(temp_proj2[0, :].round(),
                                                                         dtype='int64')), dims=img.shape[:-1],
        order='F')
    ...
    frontal_raw[ind_frontal, :] = cv2.remap(img, temp_proj2[0, :].astype('float32'), temp_proj2[1, :].astype('float32'),
                                            cv2.INTER_CUBIC)
    ...
    if np.abs(sum_diff) > ACC_CONST:  # one side is ocluded
        ...
        frontal_sym = np.multiply(frontal_raw, weights) + np.multiply(frontal_raw, weight_take_from_org) + np.multiply(
